...
```{r grnerate.bigrams}
mybigrams <- docs %>% unnest_tokens(bigram, text, token = "ngrams", n = 2) 
```
```{r filter.bigrams}
library(tidyr)
...
```{r top.bigrams}
top_bigrams <- bigrams_filtered %>% count(word1, word2, sort = TRUE)
```


```{r trigrams}
mytrigrams <- docs %>% unnest_tokens(trigram, text, token = "ngrams", n = 3) 
```
```{r filter.trigrams}
...
```{r top.trigrams}
top_trigrams <- trigrams_filtered %>% count(word1, word2, word3, sort = TRUE)
```


```{r quadgrams}
top_quadgrams <- docs %>% unnest_tokens(quadgram, text, token = "ngrams", n = 4)  %>%
